...
//require ( get_stylesheet_directory() . '/PageTempale_InstallationRequestForm/function.php' );
	...
	define('MAILGUN_URL', 'https://api.mailgun.net/v3/.mailgun.org');
	define('MAILGUN_KEY', ''); 
...
function formaspace_form_to_email(){
	...
	$site_path	= wp_normalize_path(   get_home_path()   );
	$theme_path	= get_stylesheet_directory(); 

	include_once( $theme_path . "/google-api-analytices/vendor/autoload.php");
	// require_once( $theme_path . "/google-api-analytices/models/ApihelperModel.php");
	// require_once( $theme_path . "/google-api-analytices/models/GoogleanalyticsModel.php");

	require_once( $site_path . 'wp-includes/class-phpmailer.php'  );
	require_once( $site_path . 'wp-includes/class-smtp.php'  );

	$mail 					= new PHPMailer(true);
	$mail->IsSMTP();
	$mail->Host 			= "smtp.mailgun.org";
	$mail->SMTPAuth			= true;
	$mail->Username			= 'postmaster@mailgun.formaspace.com';
	$mail->Password			= '27d1ae2a91abe0c8616c71bb80e5b067';
	$mail->From				= "design.consultant@formaspace.com";
	$mail->FromName			= "Formaspace";

	$mail->addAddress("dmitriy_r_f@mail.ru");

	$name = "Dima";
	$email = "dmitriy_r_f@mail.ru";
...
$toname									=	'Dima';
$mailfromnane							=	'example';
$mailfrom								=	'design.consultant@formaspace.com';
$mailfrom								=	'postmaster@sandbox2daaae82aa42408996b8bde97e4c93e5.mailgun.org';
$subject								=	'subject';
$html									=	$message;
$tag									=	'tag';
	    ...
	    curl_close($session);
	    $curl_results = json_decode($response, true);
